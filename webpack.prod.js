...
module.exports = merge(common, {
  ...
  devtool: 'source-map',
  output: {
    // 这里添加contentHash，由于我们的entry中没有配置入口的名称，webpack会默认取名为main，因此这里的配置会生成 main.XXXXX.js
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        use: {
          // 这是使用url-loader替代file-loader
          loader: 'url-loader',
          options: {
            // 当图片小于8kb时，url-loader会将图片转换为base64
            // 这样可以减少http请求的数量
  ...
  optimization: {
    ...
    splitChunks: {
      ...
      // 配合上面的maxInitialRequests: Infinity
      // 小于80kb的所有thunk会被打包在一起
      // 这样可以减少初始请求数
      // 根据自己情况设置
      minSize: 80 * 1024,
      // 抽离多入口引用次数为1以上的thunk
      ...
      cacheGroups: {
        ...
        vendor: {
          test: /[\\/]node_module[\\/]/,
          // 根据路径获得第三方库的名称
          // 并将抽离的thunk以 vendor_thirdPartyLibrary 格式命名
  ...
  plugins: [
    ...
    // 确保在文件没发生变化的时候，contentHash也不会变化
